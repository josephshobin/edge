...
import com.twitter.scalding.{TextLine => _, _}
...
object PartitionUtil {
  ...
  def converter[P, T, U >: (P, T)]
    (valueConverter: TupleConverter[T], partitionConverter: TupleConverter[P]) = {
  ...
  def setter[P, T, U <: (P, T)](valueSetter: TupleSetter[T], partitionSetter: TupleSetter[P])
      : TupleSetter[U] =
